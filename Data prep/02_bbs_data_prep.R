...
Species <- Soecies %>% select(AOU, English_Common_Name)
...
r <- r %>% select(placeholder, StateNum, Route) # Select only the columns of interest
...
dd_long <- reshape(dd, v.names = "Count", varying = 7:17, timevar = "Stop", times = names(dd)[7:17], direction='long')

# Get rid of extra text string in the Stop field
dd_long$Stop <- str_remove(dd_long$Stop, "Stop")


write.csv(dd_long, "dd_long.csv")
...
dd_long <- read.csv("~/thesis-july/bbs-data-prep/dd_long.csv")
...
df <- merge(dd_long, forestcodes, by = "English_Common_Name")
...
Stops <- read.csv("stopsInForest.csv")   ## if doing forest birds at forest stops
# Stops <- read.csv("stopsInOpen.csv")   ## if doing open birds at open stops

# Quick dataset clean-up of .csv file, obtained in code 02
Stops$routes = gsub('0+$', '', Stops$routes)
Stops$RouteNumber <- sub("\\.[0-9]+$", "", Stops$routes)
Stops$Stop <- sub(".*\\.", "", Stops$routes)
...
Rte <- Stops %>% filter(RouteNumber == i )

  if(nrow(s) > 0){
    s <- unique(Rte$Stop) # Identify stops (from 1 to 11) that are forested (or open) at route i
    f <- ddf %>% filter( RouteNumber == i ) %>% filter( Stop %in% stop ) # In the base dataset, for route i, keep only the stops that are forested
  ...
  if(! i %% 100){
    print(paste0("Progress: ", round(which(rteno == i)/nrow(rteno)*100, 2), "% finished."))
    flush.console()
  }
}
...
summarize_df <- summarize_df %>% filter(!NumForestStops == 0)

# Tabulate total responses across stops for each route 
# summarize_df <- summarize_df_clean %>% group_by(Transect, RouteNumber, Year, CountryNum, NumForestStops) %>% summarize(Richness = sum(Richness))
summarize_df <- summarize_df %>% group_by(Transect, RouteNumber, Year, CountryNum, NumForestStops) %>% summarize(TA = sum(TA))

# Write to csv to save progress
write.csv(summarize_df, "summarize_df.csv")
